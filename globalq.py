...
class GlobalQ(nn.Module):
    ...
    def forward(self, batch, hidden_state, select=False):
        inputs, bs = self._build_inputs(batch, select)
    ...
    def _build_inputs(self, batch, select=False):
        if select:
            bs = 1
        else:
            bs = self.batch_size
        ...
        if select:
            agent_mask = torch.zeros_like(agent_mask)
